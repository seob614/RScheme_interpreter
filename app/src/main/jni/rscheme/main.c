...
void rs_exit(){
    //get_start 매개변수 자리에  init_scheme으로 나온 초기값으로 새로지정 eval_proc에 지속적으로 push해줬었던 느낌임
    eval_proc = call_scheme(get_start, 3, NIL_OBJ, FALSE_OBJ, FALSE_OBJ);
}


obj result;
//jni function
JNIEXPORT jstring JNICALL Java_com_example_rscheme_1interpreter_Rs_eval(JNIEnv * env, jobject obj, jstring str) {
    jstring ret;
...
//init
JNIEXPORT void JNICALL Java_com_example_rscheme_1interpreter_Rs_init(JNIEnv* env, jobject thiz) {
...
JNIEXPORT void JNICALL Java_com_example_rscheme_1interpreter_Rs_exit(JNIEnv * env, jobject obj) {
    rs_exit();
}
